...
namespace Scripts.Stacks
{
    public class LabelStack : MonoBehaviour
    {
        ...
        private void Update()
        {
            _cardArrayOffset = topicStack.GetComponent<TopicStack>().cardArrayOffset;
            MoveCards();
        }
        ...
        private void MoveCards()
        {
            ...
            for (var i = 0; i < cards.Length; i++)
            {
                ...
                if (Mathf.Abs(cards[i].localPosition.y - _cardPositions[i + stackScale + _cardArrayOffset].y) < 0.01f)
                {
